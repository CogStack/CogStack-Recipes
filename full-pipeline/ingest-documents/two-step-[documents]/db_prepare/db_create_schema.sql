...
CREATE TABLE medical_reports (
	...
	binarydoc BYTEA NOT NULL									-- the binary document to be processed by Tika
) ;

CREATE TABLE medical_reports_processed (
	cid INTEGER REFERENCES medical_reports,
	dct TIMESTAMP NOT NULL,
	output TEXT													-- the extracted text will be processed by NLP
) ;
...
CREATE TABLE observations (										-- needed for CogStack Pipeline ingestion:
	cid SERIAL PRIMARY KEY,															-- > primary key  
	created TIMESTAMP DEFAULT CURRENT_TIMESTAMP + RANDOM() * INTERVAL '5 years', 	-- > timestamp
	date DATE NOT NULL, 
	patient UUID REFERENCES patients,
...
/*
  ...
  Create views for CogStack Pipeline

*/

/*
	A helper view to check the extracted text from binary documents
*/
...
/*
	The main view from which the data will be ingested from by CogStack Pipeline
*/
CREATE VIEW observations_view AS
	 SELECT
		...
		doc_proc.output::JSON ->> 'tika_output' AS document_content
	FROM 
		patients p, 
		encounters enc,
		observations obs,
